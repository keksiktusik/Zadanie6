...
public:
    ...
    // Funkcja dodaj¹ca dane (LineData) do odpowiednich wêz³ów drzewa na podstawie daty i godziny
    void addData(const LineData& lineData);

    // Funkcja do wypisania wszystkich danych w drzewie na standardowe wyjœcie
    void print() const;

    // Funkcja zwracaj¹ca dane z okreœlonego przedzia³u czasowego
    std::vector<LineData> getDataBetweenDates(const std::string& startDate, const std::string& endDate) const;

    // Funkcja do obliczenia sum dla ró¿nych kategorii (autokonsumpcja, eksport, itp.) w zadanym przedziale czasowym
    ...
    // Funkcja do obliczenia œrednich wartoœci dla ró¿nych kategorii w zadanym przedziale czasowym
    ...
    // Funkcja porównuj¹ca dane w dwóch ró¿nych przedzia³ach czasowych i obliczaj¹ca ró¿nice
    ...
    // Funkcja wyszukuj¹ca rekordy w okreœlonym przedziale czasowym, które maj¹ wartoœæ zbli¿on¹ do zadanej (z tolerancj¹)
